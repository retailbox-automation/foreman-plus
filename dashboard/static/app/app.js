...
const api = {
  ...
  triage: id => getJSON(`/api/property/${encodeURIComponent(id)}/triage`),
};
...
async function intro() {
  ...
  return el(`<section class="intro">
    ...
    <div class="veo-strip">
      <video src="/static/app/assets/hero.mp4" poster="/static/app/assets/hero-poster.jpg"
        autoplay muted loop playsinline aria-label="A field technician reading a job card on site"></video>
      <div>
        <b>Capture happens where the wrench is.</b>
        <p>A photo and a spoken sentence are the whole interface — phone today, camera glasses when hands are full.
        This field scene is generated with <span class="mono">veo-3.1-generate-001</span> on Vertex AI
        (<span class="mono">scripts/generate_hero_clip.py</span>); the ambient track judges can play while
        the demo fleet works comes from <span class="mono">lyria-002</span>.</p>
      </div>
    </div>
    <dl class="spec">
      <div><dt>Fleet</dt><dd>${esc(agents.length || 3)} ADK agents · foreman, estimator, closer</dd></div>
      <div><dt>Models</dt><dd class="mono">gemini-3.7-flash · embedding-2 · gemma-4 · veo-3.1 · lyria-002</dd></div>
...
/* Advisory Gemma triage: labels arrive AFTER first paint so a slow model never
   delays the record. The gate stays deterministic — these chips are labeled
   advisory and never touch memory. */
function attachTriage(id, node) {
  api.triage(id).then(t => {
    const labels = (t && t.labels) || {};
    let placed = 0;
    Object.entries(labels).forEach(([gid, l]) => {
      const top = node.querySelector(`[data-oq="${CSS.escape(gid)}"] .oq-top`);
      if (!top || top.querySelector('.triage')) return;
      const tag = el(`<span class="triage ${esc(l.category)}"
        title="Advisory triage by Gemma 4 (${esc(t.model)}) on Vertex AI — a reading layer over the record; it never writes memory">${esc(l.category)} · ${esc(l.urgency)}</span>`).firstElementChild;
      const meta = top.querySelector('.oq-meta');
      top.insertBefore(tag, meta || null);
      placed++;
    });
    if (placed) {
      const count = node.querySelector('#blk-oq .count');
      if (count) count.textContent += ` · triaged by ${t.model} (advisory)`;
    }
  }).catch(() => { /* advisory by contract: no labels, page unchanged */ });
}
...
async function property(id) {
  ...
  const node = el(`<div class="page" data-property="${esc(p.id)}">
...
function runLine(host) {
  ...
  line.innerHTML = '<span class="spin" aria-hidden="true"></span>' +
                   '<span id="runTxt">starting the fleet…</span>' +
                   '<button class="sndbtn" id="runSnd" type="button" aria-pressed="false" ' +
                   'title="A 30-second ambient loop generated by lyria-002 on Vertex AI (scripts/generate_wait_track.py) — the fleet takes a couple of minutes">♪ Lyria while you wait</button>' +
                   '<span class="t" id="runT">0s</span>';
  const blk = $('#blk-brief', host) || host.firstElementChild;
  blk.insertBefore(line, blk.firstChild);
  return line;
}

/* Ambient wait track — generated once by lyria-002 (Vertex AI), served static.
   Strictly opt-in: browsers block autoplaying audio anyway, and a judge who
   wants silence keeps it. */
let waitAudio = null;
function toggleWaitAudio(btn) {
  if (!waitAudio) {
    waitAudio = new Audio('/static/app/assets/lyria-wait.mp3');
    waitAudio.loop = true;
    waitAudio.volume = 0.45;
  }
  if (waitAudio.paused) {
    waitAudio.play().catch(() => {});
    btn.setAttribute('aria-pressed', 'true');
    btn.textContent = '♪ playing — Lyria-002';
  } else {
    stopWaitAudio();
  }
}
function stopWaitAudio() {
  if (waitAudio && !waitAudio.paused) waitAudio.pause();
  const b = $('#runSnd');
  if (b) { b.setAttribute('aria-pressed', 'false'); b.textContent = '♪ Lyria while you wait'; }
}
...
document.addEventListener('click', e => {
  ...
  const snd = e.target.closest('#runSnd');
  if (snd) { toggleWaitAudio(snd); return; }
