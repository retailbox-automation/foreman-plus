...
- ⏳ pending external: код кредитов $150 (форма подана 18.08, ≤72 бизнес-ч, redeem до 03.09; на 19.08 письма НЕТ — проверен весь inbox) — чек: `gmail_imap.py search --account oskola7 --query 'SUBJECT "credit"' --days 4`. Дальше: мультимодальный intake через ADK Runner (риск-точка №1 из INDEX) → pgvector recall → Firestore.
