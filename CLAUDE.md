...
- ⏳ pending external: код кредитов $150 (форма подана 18.08, ≤72 бизнес-ч, redeem до 03.09; на 19.08 письма НЕТ — проверен весь inbox) — чек: `gmail_imap.py search --account oskola7 --query 'SUBJECT "credit"' --days 4`. Дальше: дашборд (React Flow + activity feed) → observability (OTel) → A2A.
